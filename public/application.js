...
var onNewGame = function onNewGame(event) {
  api.newGame().then(ui.createGameSuccess).then(onGameHistory).catch(ui.failure);
};

var onGameHistory = function onGameHistory() {
  api.getGames().done(ui.getGamesSuccess).fail(ui.failure);
};

var apiAddHandlers = function apiAddHandlers() {
  $('#newGame').on('click', onNewGame);
  $('#showGames').on('click', onGameHistory);
};

module.exports = {
  apiAddHandlers: apiAddHandlers,
  onNewGame: onNewGame,
...
var apiEvents = __webpack_require__(3);
...
var onSignUp = function onSignUp(event) {
  ...
  }).then(ui.logInSuccess).catch(ui.signUpFailure);
};

var onLogIn = function onLogIn(event) {
  event.preventDefault();
  var data = getFormFields(this);
  api.logIn(data).done(ui.logInSuccess).fail(ui.logInFailure);
};

var onChangePw = function onChangePw(event) {
  event.preventDefault();
  var data = getFormFields(this);
  api.changePw(data).done(ui.changePwSuccess).fail(ui.changePwFailure);
};
...
var authAddHandlers = function authAddHandlers() {
...
module.exports = {
  authAddHandlers: authAddHandlers
};
/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(0)))
...
var showWinModal = function showWinModal() {
...
var checkWinner = function checkWinner() {
  if (store.gameboard[0] !== '' && store.gameboard[0] === store.gameboard[1] && store.gameboard[0] === store.gameboard[2] || store.gameboard[3] !== '' && store.gameboard[3] === store.gameboard[4] && store.gameboard[3] === store.gameboard[5] || store.gameboard[6] !== '' && store.gameboard[6] === store.gameboard[7] && store.gameboard[6] === store.gameboard[8] || store.gameboard[0] !== '' && store.gameboard[0] === store.gameboard[3] && store.gameboard[0] === store.gameboard[6] || store.gameboard[1] !== '' && store.gameboard[1] === store.gameboard[4] && store.gameboard[1] === store.gameboard[7] || store.gameboard[2] !== '' && store.gameboard[2] === store.gameboard[5] && store.gameboard[2] === store.gameboard[8] || store.gameboard[0] !== '' && store.gameboard[0] === store.gameboard[4] && store.gameboard[0] === store.gameboard[8] || store.gameboard[2] !== '' && store.gameboard[2] === store.gameboard[4] && store.gameboard[2] === store.gameboard[6]) {
    over = true;
...
var onClick = function onClick() {
  $('.box').on('click', function (event) {
    ...
    if (currentCell.text() === '') {
      ...
      if (moves % 2 !== 0) {
        currentCell.text('X');
        var value = currentCell.text();
        var index = currentCell.attr('data-index');
        store.gameboard[index] = currentPlayer;
        checkWinner();
        api.updateGame(index, value, over).done(ui.updateGameSuccess).catch(ui.failure);
      } else {
        currentCell.text('O');
        var _value = $(this).text();
        var _index = currentCell.attr('data-index');
        store.gameboard[_index] = currentPlayer;
        checkWinner();
        api.updateGame(_index, _value, over).done(ui.updateGameSuccess).catch(ui.failure);
      }
      return over;
    }
  });
